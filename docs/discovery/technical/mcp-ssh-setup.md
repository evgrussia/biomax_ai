...
После настройки MCP сервера, агенты могут использовать SSH MCP для управления VPS.
...
**DevOps Agent** и **SRE Agent** могут использовать MCP сервер `user-biomax-vps` для:
- Выполнения команд на сервере
- Передачи файлов (upload/download)
- Мониторинга сервисов
- Деплоя приложений

**Навык:** Используй навык `vps-management` (`.cursor/skills/vps-management/SKILL.md`) для детальных паттернов работы с VPS через MCP.

**Пример использования:**
```
Агент может:
1. Проверить текущий статус: MCP execute-command "docker-compose ps"
2. Если есть проблемы - проанализировать логи: MCP execute-command "docker-compose logs"
3. Принять решение о деплое или откате
4. Выполнить необходимые действия через MCP
5. Проверить результат
```

### Интеграция с существующими скриптами

MCP может заменить или дополнить:
- `deploy_vps.py` — можно использовать MCP для прямого управления
- `deploy_final.py` — можно интегрировать с MCP для проверок
- `deploy.sh` — можно выполнять через MCP execute-command
