...
description: |
  Senior Analytics Engineer агент для определения метрик и tracking.
  Использовать когда: нужно создать tracking plan, определить метрики, 
  спланировать A/B эксперименты, создать формулы расчёта KPI.
---
